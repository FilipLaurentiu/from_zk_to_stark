...
impl<'a> Div for Polynomial<'a> {
    ...
    fn div(self, rhs: Polynomial<'a>) -> Self::Output {
        ...
        let mut result_coefficients = vec![
...
impl<'a> Polynomial<'a> {
    ...
    pub fn lagrange_interpolation(
        points: &[(FieldElement<'a>, FieldElement<'a>)],
        finite_field: &'a FiniteField,
    ) -> Self {
        ...
        for (i, i_element) in points.iter().enumerate() {
            ...
            for (j, j_element) in points.iter().enumerate() {
                ...
                let basis = (x - &Polynomial::new([j_element.0].to_vec(), finite_field))
                    * Polynomial::new(
                        [(i_element.0 - j_element.0).inverse()].to_vec(),
                        finite_field,
                    );
                value = value * basis;
            }
...
mod tests {
    ...
    fn test_mul_polynomial() {
        ...
        assert_eq!(
            ...
            Polynomial::from_slice(&[6, 31, 56, 35], &finite_field)
        );
    }
    ...
    fn test_div_polynomial() {
        ...
        assert_eq!(
            division.0,
            Polynomial::from_slice(&[40, 84, 1], &finite_field)
        );
    }
